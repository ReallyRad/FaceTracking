...
namespace Calcatz.Sequine {
    ...
    /// <summary>
    /// Base class for property commands that targets a Sequine Player component.
    /// </summary>
    [System.Serializable]
    public class SequineTargetPropertyCommand : PropertyCommand {
        ...
        protected SequinePlayer GetSequinePlayer(CommandExecutionFlow _flow) {
            if (inputIds[IN_PORT_TARGET].targetId != 0) {
                ...
                if (inputObject is SequinePlayer sequinePlayer) {
                    ...
                    Debug.LogError("Error at command ID " + id + ": The connected input towards the Target is neither a Sequine Player component nor Binder Asset.");
                    return null;
                }
            }
            else {
            ...
            Debug.LogError(this.GetType().Name + ": Can't get the Sequine Player, make sure it is assigned.");
            return null;
        }
